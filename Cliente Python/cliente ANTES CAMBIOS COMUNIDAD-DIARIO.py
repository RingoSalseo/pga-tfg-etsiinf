...
#PARA AÑADIR ESTACIONALIDAD CALCULAMOS EL NUMERO DE LA SEMANA
#Despues calculamos dos numeros que representan el nº de la semana
#df_cero_cp["semana"] = df_cero_cp["fecha"].dt.isocalendar().week.astype(int)
#df_cero_cp["sin_semana"] = np.sin(2 * np.pi * df_cero_cp["semana"] / 52)
#df_cero_cp["cos_semana"] = np.cos(2 * np.pi * df_cero_cp["semana"] / 52)
...
data = df_cero_cp
hyperparameter_search = False # <<<<<<<< CAMBIA A True PARA BUSQUEDA

# Escalar datos por separado para evitar fuga de información
...
# Extraer las ventas escaladas para facilitar el manejo
scaled_ventas_train = scaled_train_data[:, ventas_column_index].reshape(-1, 1)
scaled_ventas_test = scaled_test_data[:, ventas_column_index].reshape(-1, 1)
...
# Compilar el modelo
model.compile(optimizer="adam", loss="mean_squared_error") #Se puede usar mean_squared_error y mean_absolute_error
...
# Invertir el escalado de las predicciones y de los valores reales
...
#CALCULAMOS EL ERROR CUADRATICO MEDIO. COMUN
